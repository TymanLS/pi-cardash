...
### GPS Tab
mapSurf = pygame.image.load('Map.png')
mapRect = mapSurf.get_rect(center=(gui.SCREEN_WIDTH/2,(gui.SCREEN_HEIGHT/2)+20))
min_long = -111.9087
max_long = -111.8209
max_lat = 33.2752
min_lat = 33.2352
...
try:
        while time.time() - start_time < timeout:
                ...
                #test gps values
                lat = max_lat-(count/100000)
                lon = min_long+(count/100000)
                ...
                # Draw GPS tab
                    ...
                    y=(map_height/(max_lat-min_lat))*(max_lat-lat)
                    x=(map_width/(min_long-max_long))*(min_long - lon)
                    #update for screen coord
                    x=(gui.SCREEN_WIDTH/2 - map_width/2) + x
                    y=(gui.SCREEN_HEIGHT/2 - map_height/2 + 20) + y
                    pygame.draw.circle(screen, gui.colors.GREEN, (int(x),int(y)),5)
                # Draw Dashcam tab
